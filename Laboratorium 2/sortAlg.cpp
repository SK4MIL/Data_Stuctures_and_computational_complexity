...
//TODO: FIX IT;
void bucketSort(vector<int> data, int aSize){
    //TODO: czy to zadziała>
    vector<int> bucket[4];

    cout<<"Przypisalem te buckety "<<aSize<<"\n";
    // Przydzielamy liczby do odpowiednich wiaderek. 
    for (int i=0; i < aSize; i++)
    {
        int buckid = aSize*data[i];
        bucket[buckid].push_back(data[i]);
        cout<<"pushed back at bucket "<<buckid<<" value: "<<data[i]<<"\n";
    }
    cout<<"Rozdzielilem liczby\n";
...
void mixedBucket(int data[], int aSize){
    ...
    //TODO: Czy na pewno size?
    // if (Negative.size() > 0){ bucketSort(Negative, (int)Negative.size()); 
    // cout<<"Negatywny poszedł\n";}
    // if (Positive.size() > 0){
    //     cout<<"Pozytywny poszedł\n";
    //     bucketSort(Positive, (int)Positive.size());
    // }
    cout<<"Wrocily\n";
    for (int i=1; Negative.size(); i++){
        data[i-1] = -1* Negative[Negative.size() - i];
    }
